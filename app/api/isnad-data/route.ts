...
export const GET = async (req: Request, res: Response) => {
  try {
    ...
    const scholars = await conn.execute(
      `SELECT id, name, bornAt, diedAt, livedIn, nickName FROM scholar WHERE id IN (?) ORDER BY FIELD(id, ?)`,
      [isnadParsed as number[], isnadParsed as number[]]
    );
